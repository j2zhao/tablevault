...
@pytest.mark.parametrize("funct_name", [
    "create_code_module",
    "delete_code_module",
    "create_builder_file",
    "delete_builder_file",
...
def test_setup_exception(tablevault:TableVault, funct_name):
    ...
    with pytest.raises(tv_errors.TableVaultError):
        with patch.dict('tablevault._operations._meta_operations.SETUP_MAP', {funct_name:raise_tv_except}):
...
def test_exception(tablevault:TableVault, funct_name):
    process_ids = []
    with pytest.raises(tv_errors.TableVaultError):
        with patch('tablevault._operations._vault_operations.' + funct_name, raise_tv_except):
            process_ids = []
            base_exception_function(tablevault, process_ids)
    evaluate_operation_logging(process_ids)
    assert compare_folders('example_tv', 'example_tv_copy')
