...
from tablevault._prompt_execution.parse_code import execute_code_from_prompt, execute_gen_table_from_prompt
...
def _fetch_table_cache(external_dependencies:list, db_metadata:MetadataStore, instance_id:str, table_name:str, db_dir:str,
    ...
    for dep in external_dependencies:
        ...
        if latest:
            ...
            cache[(table, instance)] = _file_operations.get_table(instance, table, db_dir)
    return cache
...
def execute_table(table_name: str, db_dir: str, author: str, instance_id: str = '', force: bool = False): #TODO: lock origin instance, edit prompt parser
    ...
    top_pnames, to_execute, to_change_columns, all_columns, internal_prompt_deps, external_deps = prompt_parser.parse_prompts(prompts, db_metadata , start_time, instance_id, table_name, db_dir, force)
    if origin_lock != None:
    ...
    data = {'origin': origin, 'perm_instance_id': perm_instance_id, 'to_execute': to_execute,
    ...
    for i, pname in enumerate(top_pnames):
        ...
        if i == 0:
            rows_updated = execute_gen_table_from_prompt(prompt, cache, instance_id, table_name, db_dir)
    ...
    if not rows_updated and len(to_change_columns) == 0:
        ...
        _file_operations.materialize_table_folder(perm_instance_id, instance_id, table_name, db_dir)
...
def restart_execute_table(author:str, process_id:str, db_dir:str):
    ...
    if 'stop_execute' in process.complete_steps:
        ...
        instance_lock.release_exclusive_lock()
        return
    perm_instance_lock = DatabaseLock(db_dir, table_name, perm_instance_id, restart=True)
    perm_instance_lock.acquire_exclusive_lock()
    ...
    for pname in external_deps:
        for table, _, instance, _,_ in external_deps[pname]:
            lock = DatabaseLock(db_dir, table_name=table, instance_id=instance, restart=True)
            lock.acquire_shared_lock()
            dep_locks.append(lock)
    ...
    for i, pname in enumerate(top_pnames):
        ...
        if i == 0:
            ...
            if rows_updated or set(prompt['parsed_changed_columns']).issubset(to_change_columns):
                if prompt['type'] == 'code':
                    execute_code_from_prompt(prompt, cache, instance_id, table_name, db_dir)
                elif prompt['type'] == 'llm':
                    execute_llm_from_prompt(prompt, cache, instance_id, table_name, db_dir)
...
def setup_table_instance(version: str, table_name: str, db_dir: str, author: str, 
                         origin_id: str = '',
                         prompts: list[str] = [],
                         gen_prompt: str = ''):
    ...
    process_id = db_metadata.start_new_process(author, 'setup_table_instance', table_name, instance_id, data= data, start_time=start_time)
    _file_operations.setup_table_instance_folder(instance_id, table_name, db_dir, origin_id, prompts, gen_prompt) 
    db_metadata.write_to_log(process_id)
...
def restart_setup_table_instance(author:str, process_id:str, db_dir:str):
    ...
    lock = DatabaseLock(db_dir, table_name, instance_id, restart=True)
    lock.acquire_exclusive_lock()
    _file_operations.setup_table_instance_folder(instance_id, table_name, db_dir, origin, prompts, gen_prompt) 
    db_metadata.write_to_log(process_id)
    lock.release_exclusive_lock()
...
def restart_database(author:str, db_dir: str, excluded_processes: list[str] = []):
    ...
    db_metadata = MetadataStore(db_dir)
    active_logs = db_metadata.get_active_processes()
    data = {'excluded_processes': excluded_processes, 'active_ids': list(active_logs.keys())}
