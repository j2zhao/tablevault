...
class MultiLock:
    ...
    def acquire_shared(
        ...
        :param timeout: Maximum time (in seconds) to wait for the lock. None means wait
        indefinitely.
        ...
        while True:
            try:
                ...
                portalocker.lock(
                    self.read_handle, portalocker.LOCK_SH | portalocker.LOCK_NB
                )
    ...
    def acquire_exclusive(
        ...
        while True:
            try:
                ...
                portalocker.lock(
                    self.write_handle, portalocker.LOCK_EX | portalocker.LOCK_NB
                )
                return True  # Lock acquired
            except (portalocker.exceptions.LockException, IOError):
                # Failed to acquire lock
...
class DatabaseLock:
    def __init__(
        ...
        if instance_id:
            ...
            self.locks.append(
                MultiLock(os.path.join(base_lock_dir, f"{table_name}.lock"))
            )
            ...
            for root, _, files in os.walk(base_lock_dir):
                for file in files:
                    if (
                        file.endswith(".lock")
                        and file != "DATABASE.lock"
                        and file != "RESTART.lock"
                    ):
                        lock_path = os.path.join(root, file)
                        self.locks.append(MultiLock(lock_path))
            self.type = "db"
    ...
    def _delete_locks(self) -> None:
        if self.type == "db":
            ...
            lock_path = os.path.join(
                self.db_dir, "locks", self.table_name, f"{self.instance_id}.lock"
            )
            os.remove(lock_path)
        self.deleted = True

    def release_shared_lock(self, delete: bool = False) -> None:
        with self.lock:
            if self.deleted:
                raise ValueError("Lock Does Not Exist")
    ...
    def release_exclusive_lock(self, delete: bool = False) -> None:
        with self.lock:
            if self.deleted:
                raise ValueError("Lock Does Not Exist")
            if self.has_lock is None:
