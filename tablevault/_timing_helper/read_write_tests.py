...
def write_and_read_pickle(data_dict, filename="test.pkl"):
    with open(filename, "wb") as f:
        _, write_time = time_function(pickle.dump, data_dict, f, protocol=pickle.HIGHEST_PROTOCOL)
    file_size = get_file_size(filename)
...
def write_and_read_parquet(df, filename="test.parquet"):
    ...
    _, write_time = time_function(df.to_parquet, filename, engine=PARQUET_ENGINE, index=False)
    file_size = get_file_size(filename)
...
for N in test_sizes:
    ...
    data_list = [{"number": i, "text": "This is a sample string. " * 5} for i in range(N)]
    ...
    parquet_w, parquet_r, parquet_size = write_and_read_parquet(df) if PARQUET_ENGINE else (None, None, None)
    hdf_w, hdf_r, hdf_size = write_and_read_hdf(df)
    feather_w, feather_r, feather_size = write_and_read_feather(df) if PARQUET_ENGINE else (None, None, None)
    ...
    print(f"JSON:           write = {result['json_write_ms']:.2f} ms, read = {result['json_read_ms']:.2f} ms, file size = {result['json_file_size_bytes']} bytes")
    print(f"Pickle:         write = {result['pickle_write_ms']:.2f} ms, read = {result['pickle_read_ms']:.2f} ms, file size = {result['pickle_file_size_bytes']} bytes")
    print(f"YAML:           write = {result['yaml_write_ms']:.2f} ms, read = {result['yaml_read_ms']:.2f} ms, file size = {result['yaml_file_size_bytes']} bytes")
    print(f"CSV (csv mod):  write = {result['csv_module_write_ms']:.2f} ms, read = {result['csv_module_read_ms']:.2f} ms, file size = {result['csv_module_file_size_bytes']} bytes")
    print(f"Pandas CSV:     write = {result['pandas_csv_write_ms']:.2f} ms, read = {result['pandas_csv_read_ms']:.2f} ms, file size = {result['pandas_csv_file_size_bytes']} bytes")
    if parquet_w is not None:
        print(f"Parquet ({PARQUET_ENGINE}): write = {result['parquet_write_ms']:.2f} ms, read = {result['parquet_read_ms']:.2f} ms, file size = {result['parquet_file_size_bytes']} bytes")
    else:
        print("Parquet:        skipped (no engine installed)")
    print(f"HDF5:           write = {result['hdf_write_ms']:.2f} ms, read = {result['hdf_read_ms']:.2f} ms, file size = {result['hdf_file_size_bytes']} bytes")
    if feather_w is not None:
        print(f"Feather:        write = {result['feather_write_ms']:.2f} ms, read = {result['feather_read_ms']:.2f} ms, file size = {result['feather_file_size_bytes']} bytes")
    else:
