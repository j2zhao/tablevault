...
def get_function_from_module(module_name: str, function_name: str) -> Callable: # wierd bug here -> TODO: fix
    if function_name == 'create_paper_table_from_folder' or function_name == 'create_data_table_from_table':

        module_name = 'tablevault._code_functions.' + 'table_generation'
    else:
        module_name = 'tablevault._code_functions.' + 'open_ai_store'
    # Import the module by its absolute name
    module = import_module(module_name)
    #module = import_module('tablevault._code_functions.table_generation')
...
def execute_code_from_prompt(prompt:prompt_parser.Prompt, cache:  prompt_parser.Cache,
    ...
    if is_global:
        ...
        #code_file =   'table_generation'
        #module_name = 'tablevault._code_functions.' + code_file

        module_name = 'tablevault._code_functions.table_generation'
        funct = get_function_from_module(module_name, prompt_function)
    else:
...
def execute_gen_table_from_prompt(prompt:prompt_parser.Prompt, cache: prompt_parser.Cache, 
    ...
    if is_global:
        module_name = f'tablevault._code_functions.{code_file}'
        funct = get_function_from_module(module_name, prompt_function)
    else:
        code_file = os.path.join(db_dir, 'code_functions')
        code_file = os.path.join(code_file, code_file)
    ...
    df = pd.merge(results, cache['self'], how='left', on=prompt['parsed_changed_columns'])
    df = df[columns]
    
    if not df[prompt['parsed_changed_columns']].equals(cache["self"][prompt['parsed_changed_columns']]):
        _file_operations.write_table(df, instance_id, table_name, db_dir)
        return True
    else:
        return False