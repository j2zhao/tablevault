...
def _execute_llm(index: int, prompt: dict, client: Optional[openai.OpenAI], 
    ...
    name = prompt['name'] + str(index) + ''.join(random.choices(string.ascii_letters, k=5))
    if 'context_files' in prompt:
        context_files = prompt_parser.get_table_value(prompt['context_files'], index,cache)
    else:
        context_files = []
    if 'context_msgs' in prompt:
        context_msgs = prompt_parser.get_table_value(prompt['context_msgs'], index, cache)
    else:
        context_msgs = ''
    if 'instructions' in prompt:
        instructions = prompt_parser.get_table_value(prompt['instructions'], index,cache)
    else:
        instructions = None
    ...
    thread = Open_AI_Thread(name, prompt['model'], prompt['temperature'], prompt['retry'],
                            instructions, client=client, uses_files = uses_files)
