...
from tablevault.errors import DVLockError
...
def _acquire_lock(
        process_id:str, lock_path:str,
        lock_type: str,
        timeout: Optional[float], check_interval: float
    ) -> None:
    ...
    while True:
        ...
        if timeout is not None and (time.time() - start_time) >= timeout:
            raise DVLockError("Timeout while trying to acquire read lock.")
        ...
        for filename in filenames:
            if filename.endswith('.exlock') or filename.endswith('.shlock'):
                ...
                if lock_id == lid:
                    ...
                    empty = False
        if empty:
            empty = any(os.path.isdir(os.path.join(dirpath, entry)) for entry in os.listdir(dirpath))
            if empty:
...
class DatabaseLock:
    ...
    def acquire_shared_lock(self, table_name:str = '', instance_id:str = '',
                            timeout: Optional[float] = TIMEOUT, check_interval: float = CHECK_INTERVAL) -> tuple[str, str, str]:
        if table_name != '':
    ...
    def acquire_exclusive_lock(self, table_name:str = '', instance_id:str = '',
                            timeout: Optional[float] = TIMEOUT, check_interval: float = CHECK_INTERVAL) -> tuple[str, str, str]:
