...
| --------------------------- | ---------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| **`dataframe`** *(default)* | Create **whole table** in one function                           | Function returns **one `pd.DataFrame`** that already includes **every column in `changed_columns`**.        | Writes output columns (`ColumnBuilder`) directly or joins with origin table on primary key (`IndexBuilder`)          |
...
| **`generator`**             | Generate **rows incrementally** in one function                  | Function yields **`row_tuple`** in order.                                                                   | Writes each yielded row immediately                                                                                  |
