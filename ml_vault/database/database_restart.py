        ...
        try:
            ...
            if current_time - last_update > interval:
                if op_info[0] == "create_artifact_list":
                    ...
                    artifact_collection.delete_artifact_list_inner(db, ts, op_name, op_coll_name, op_session_name, op_session_index)
                    utils.commit_new_timestamp(db, ts, "restart")
                elif op_info[0] == "session_add_code_end":
                    op_name = op_info[1]
                    op_index = op_info[2]
                    op_error = op_info[3]
                    session_collection.session_add_code_end(db, op_name, op_index, op_error, ts)
                elif op_info[0] == "session_resume_request":
                    op_name = op_info[1]
                    op_session_name = op_info[2]
                    session_collection.session_resume_request(db, op_name, op_session_name, timestamp = ts)
