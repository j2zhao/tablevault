...
def session_resume_request(db: StandardDatabase, name, action, session_name, session_pid, timeout = 5, wait_time = 0.1):
    ...
    if doc["interupt_action"] != "start_pause":
        raise ValueError(f"session status is {doc["status"]} (not puased).")
...
def session_add_code_start(db:StandardDatabase, name, code):
    session = db.collection("session")
    session_code = db.collection("session_code")
    doc = session.get({"_key":name})
...
def session_add_read_artifact_edge(db:StandardDatabase, artifact_name, artifact_type, session_name, timestamp, line_num):
