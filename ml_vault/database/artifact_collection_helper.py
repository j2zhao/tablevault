...
def delete_artifact_name(db:StandardDatabase, artifact_name):
    ...
    description.delete(artifact_name, ignore_missing=True)
...
def add_session_parent_edge(db, artifact_key, artifact_collection, parent_name, timestamp):
    edge = db.collection("session_parent_edge")
    doc = {
        "_key": artifact_key,
        "timestamp": timestamp, 
        "_from": f"session_list/{parent_name}",
